...
class SmtpProxySSL(smtplib.SMTP_SSL):
    """
    SSL SMTP connection with optional proxy wrapping.
    """
    ...
    def _get_socket(self, host, port, timeout):
        """
        Create a proxied socket and wrap it with SMTP_SSL's SSL context.
        """
        if not self._proxifier:
            return super()._get_socket(host, port, timeout)
        if timeout is not None and (not timeout):
        ...
        if self.debuglevel > 0:
            self._print_debug("connect: to", (host, port), self.source_address)
        new_socket = self._proxifier.get_socket(
            self.source_address, host, port, timeout
        )
